...
function parseRecord(record, context) {
  // Fill in defaults
  record.message ||= 'Unknown event passed to Logfire AWS Lambda';
  record.level ||= 'info';
  record.data ||= {};
  record.data.context ||= {};
  ...
  const bracketLevelMatch = record.message.match(
    /^\[(DEBUG|TRACE|VERBOSE|INFO|WARN(ING)?|ERROR|CRITICAL|FATAL)]\s+/
  );
  ...
  const datetimeMatch = record.message.match(
    /^(\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(\.\d+)?(([+-]\d\d:\d\d)|Z)?)\s+/
  );
  if (datetimeMatch) {
  ...
  const requestUuidMatch = record.message.match(
    /(?:^|RequestId: )([0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12})\s+/
  );
  if (requestUuidMatch) {
  ...
  const levelMatch = record.message.match(
    /^(DEBUG|TRACE|VERBOSE|INFO|WARN(ING)?|ERROR|CRITICAL|FATAL)\s+/
  );
  if (levelMatch) {
...
async function parseAwsLogsRecords(event) {
  const awsLogsData = await new Promise((resolve, reject) => {
    zlib.gunzip(
      Buffer.from(event.awslogs.data, 'base64'),
